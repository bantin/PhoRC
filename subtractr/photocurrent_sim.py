...
def add_photocurrents_to_expt(key, expt, pc_shape_params=None,
        ...
        stim_dur_ms=5.0, pc_response_var=0.1,
    ...
    pscs = np.copy(expt['pscs'])
    psc_len = len(pscs)
    num_stims = len(expt['stim_times'])
    jittered_pc_shapes = sample_jittered_photocurrent_shapes(key, num_stims, **pc_full_params)[1]
    i = 0
    for cell_idx in pc_cells:
        ...
        for j, st in enumerate(stim_times_this_cell):
            ...
            # account for edge effects: if the photocurrent is too long, it will
            # extend beyond the end of the psc trace. In this case, we
            # truncate the photocurrent.
            k = min(jittered_pc_shapes.shape[1], psc_len - st) 
            pscs[st:st+jittered_pc_shapes.shape[1]] += jittered_pc_shapes[i,:k] * pc_amplitudes[j]
            i += 1

    obs_with_photocurrents = split_overlapping(pscs, expt['stim_times'], prior_context, response_length)
    expt['obs_with_photocurrents'] = obs_with_photocurrents
